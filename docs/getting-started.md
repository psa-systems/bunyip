...
- The [`infisical` CLI](https://infisical.com/docs/cli/overview): optional for dev (`just init-secrets` generates local throwaway values), required on a deployed host, where `just sync-secrets` renders `./secrets/*` from Infisical (see [secrets from Infisical](secrets-infisical.md)). The CLI covers Group-1 file secrets only; Group-2 integration secrets (SMTP) are fetched by the app itself at runtime with no CLI, per the same doc.
