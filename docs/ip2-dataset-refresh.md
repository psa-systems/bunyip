...
`scripts/refresh-ip2-datasets.sh` downloads and installs both files atomically. It needs `curl`, `unzip`, and a free IP2Location download token (create an account at ip2location.com; the token is on the account's download page).

```
IP2LOCATION_TOKEN=<token> DATASET_DIR=/data ./scripts/refresh-ip2-datasets.sh
```
...
0 5 3 * * deploy  . /etc/bunyip/ip2.env && DATASET_DIR=/data /opt/bunyip/scripts/refresh-ip2-datasets.sh >> /var/log/bunyip-ip2-refresh.log 2>&1
```

`/etc/bunyip/ip2.env` holds `IP2LOCATION_TOKEN=...` (mode 0600). The same command works as a systemd timer, a Kubernetes CronJob, or any scheduler: it is a plain script with a non-zero exit on failure.
