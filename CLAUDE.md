...
- **At-rest encryption**: ONE key, `APP_ENCRYPTION_KEY` (+ `APP_ENCRYPTION_KEY_PREV`, a comma-separated list of keys old rows still need, + `APP_KEY_VERSION`), protects `user_totp`, `stripe_config` and `email_config` alike (BUNYIP-483). `Config::app_key_set()` builds the one `AppKeySet`; every encrypt/decrypt site takes it. Rewrite existing rows with `bunyip-api reencrypt-secrets` (idempotent); `scripts/check-no-legacy-key-env.sh` fails the build if a per-consumer key name reappears. Runbook: `docs/encryption-key-rotation.md`.
