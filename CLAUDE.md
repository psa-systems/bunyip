...
- **Secret sourcing**: the app only ever reads `/run/secrets/*` via the `{NAME}_FILE` convention (BUNYIP-38). The files come from `scripts/init-secrets.nu` (dev throwaways) or `scripts/sync-secrets.nu` / `just sync-secrets` (rendered from the Infisical folder `/bunyip/app`, BUNYIP-504); nothing fetches a secret at runtime, so Infisical is never a boot dependency. Adding a `compose.yml` secret means adding it to the `SECRET_MAP` in `sync-secrets.nu`, whose `--self-test` (run in `check.yml`) re-derives the table from `compose.yml` and fails otherwise. Runbook: `docs/secrets-infisical.md`.
