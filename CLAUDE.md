...
- **Secret sourcing (two tiers)**: Group-1 startup secrets (postgres, `DATABASE_URL`, `APP_ENCRYPTION_KEY`, `JWT_SECRET`, ...) are file-based: the app reads `/run/secrets/*` via the `{NAME}_FILE` convention (BUNYIP-38), files rendered by `scripts/init-secrets.nu` (dev throwaways) or `scripts/sync-secrets.nu` / `just sync-secrets` (from the Infisical folder `/bunyip/app`, BUNYIP-504). No Rust code fetches a Group-1 secret at runtime, so Infisical is never needed to boot; adding a `compose.yml` secret means adding it to the `SECRET_MAP` in `sync-secrets.nu`, whose `--self-test` (run in `check.yml`) re-derives the table from `compose.yml`. Group-2 post-startup integration secrets (SMTP first) are fetched by the app from Infisical at runtime (`crates/bunyip-domain/src/services/infisical.rs`, BUNYIP-525) via a Universal Auth machine identity; the fetch is graceful (any failure leaves the feature off), so Infisical is still not a boot dependency. Runbook: `docs/secrets-infisical.md`.
