...
- `just create-release <major|minor|hotfix>` - bump `[workspace.package].version`, push the branch, open the release PR. Needs a local Rust toolchain (it runs a member-scoped `cargo update` to sync `Cargo.lock`; it is deliberately NOT `--workspace`, which would also roll the dunite git dep forward). On a toolchain-less dev box use `just create-release-container <bump>`, which runs only that one cargo step inside the pinned rust-builder image (online, to resolve the dunite-core git dep); every git/fj step stays on the host. Keep the two recipes in sync.
