        ...
        AuthShell {
            ...
            if state.read().mfa_challenge.is_some() {
                ...
                form { class: "space-y-4", onsubmit: submit_password,
                    ...
                    GoogleSsoButton { return_to: return_to.read().clone(), label: "Continue with Google" }
                    AuthInput {
                        name: "email",
                        label: "Email",
                        input_type: "email",
        ...
        AuthShell {
            ...
            if sent() {
                ...
                form { class: "space-y-4", onsubmit: submit,
                    GoogleSsoButton { return_to: None, label: "Sign up with Google" }
                    AuthInput {
                        name: "email",
                        label: "Email",
                        input_type: "email",
...
/// Continue-with-Google button. Triggers the existing PKCE OIDC flow
/// against mokosh-server with `idp_hint=google` appended so the IdP UI
/// can skip the chooser and go straight to Google. If `idp_hint` is
/// ignored by mokosh-server's IdP UI, the user simply sees the chooser
/// and picks Google there; auth still completes via the same callback.
///
/// Hidden when `BUNYIP_OIDC_ISSUER` is unset (dev builds without env
/// baked in) so the button never points at an empty issuer URL.
#[component]
fn GoogleSsoButton(
    return_to: Option<String>,
    #[props(default = "Continue with Google")] label: &'static str,
) -> Element {
    let cfg = OidcConfig::from_env();
    ...
    let click = move |_| {
        let cfg = OidcConfig::from_env();
        let rt = return_to.clone().unwrap_or_default();
        if let Err(e) = oidc::start_login_with_idp_hint(&cfg, rt, "google") {
            toast.error(format!("Could not start Google sign-in: {e}"));
        }
    };
    rsx! {
        button {
            r#type: "button",
            class: "w-full px-4 py-2.5 rounded-lg border border-bunyip-reed-200 dark:border-bunyip-reed-700 bg-white dark:bg-bunyip-reed-900 text-bunyip-reed-800 dark:text-bunyip-reed-100 font-medium hover:bg-bunyip-reed-50 dark:hover:bg-bunyip-reed-800 transition-colors flex items-center justify-center gap-2",
            onclick: click,
            // Inline Google "G" mark, no external asset fetch.
            svg {
                class: "w-4 h-4",
                view_box: "0 0 24 24",
