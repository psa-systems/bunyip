...
///
/// In a writable provider (`database`, `infisical`) it is the usual "leave blank
/// to keep it" note. In `environment` mode the field is read-only, so the note
/// names the provider that owns the value and the file to edit instead: without
/// it an admin types a key into a form whose save lands nowhere.
pub(super) fn secret_field_note(
    ...
    if !editable {
        return format!(
            "Owned by the {storage} provider (SECRETS_STORAGE={storage}), which bunyip cannot \
             write. Edit the file {env_name}_FILE points at (./secrets/{secret_file}) and \
             restart bunyip-api. Saving here returns a conflict."
        );
    }
    let provider = if storage.is_empty() {
        "declared"
    } else {
        storage
    };
    if has_value {
        format!(
            "A value is set in the {provider} provider. Leave blank to keep it, or type a new \
             one to replace it."
        )
    } else {
        format!("No value set. Saved to the {provider} provider.")
    }
}
