...
## May 31 update: scaffold filled; the consumption boundary is resolved (CONSUME)

The "Unresolved contradiction (decide before filling)" above is **resolved**, and the scaffold has been filled and merged to `main`:
...
- **Step 8 (reversed OIDC wiring) is still open:** `bunyip-web` is configured as a **client of mokosh-server** (`BUNYIP_OIDC_ISSUER` + the `register-bunyip-client` flow) even though bunyip-api is now its own issuer (`OIDC_ISSUER=:4401`). The transitional dual-issuer state and the whole dev-sso/Traefik topology are documented in `dev-docs/dev-sso-three-repo-runbook.md` (sec 3.8).
- **Billing:** a full Stripe vertical exists (ported from menkent). The M1 acceptance-criteria audit, including the a8n.tools-consumer-vs-PSA-SaaS-plan reshape and two correctness bugs, is in `dev-docs/billing-m1-gap-matrix.md`.
