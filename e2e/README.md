...
| Auth | `tests/auth/password-reset.spec.ts` | runnable | register a disposable account, request `/password-reset`, read the token from the Mailpit sink, set a new password, assert it logs in (BUNYIP-150). Skipped without `E2E_MAIL_SINK_URL` |
...
| Account | `tests/account/change-email.spec.ts` | runnable | register a disposable account, request an email change, confirm via the emailed link from the Mailpit sink, assert the new email (BUNYIP-150). Skipped without `E2E_MAIL_SINK_URL` |
...
| `E2E_MAIL_SINK_URL` | no | Staging Mailpit HTTP API base URL with basicAuth embedded (`https://user:pass@mailpit.a8n.systems`). The email-driven specs read token links here; unset (e.g. production) makes them skip (BUNYIP-150) |
...
| BUNYIP-150 (mail sink) | `auth/signup` | a programmatic mailbox can read the confirmation link. The `password-reset`, `magic-link`, and `change-email` specs now run via the Mailpit sink (`E2E_MAIL_SINK_URL`); `signup` is the remaining follow-up |
