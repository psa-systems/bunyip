...
just init-secrets            # dev throwaways (deployments supply ./secrets via the SOPS compose-secrets.yml)
docker compose up --detach
```

Group-1 startup secrets are files, never environment variables: `compose.yml` mounts each one from `./secrets/<name>` at `/run/secrets/<name>` and the api reads it through the `{NAME}_FILE` convention, so `docker inspect` never shows a value. `just init-secrets` ([`scripts/init-secrets.nu`](scripts/init-secrets.nu)) generates them locally, which is the right choice for a self-host that keeps its own values; the PSA deployments supply the same files from the SOPS `compose-secrets.yml` (docker repo, per host). Group-1 secrets are never fetched from Infisical. Group-2 integration secrets (the SMTP password and the two Stripe secrets) come from the ONE store the deployment declares in `SECRETS_STORAGE=environment|database|infisical`, and only from that store. Both tiers, the key-to-file mapping, the store declaration, and the migration and rotation flows are in [`docs/secrets-infisical.md`](docs/secrets-infisical.md).
