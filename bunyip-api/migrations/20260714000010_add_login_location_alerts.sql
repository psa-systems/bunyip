-- BUNYIP-366: detect a country-level login-location change and email the user.
--
...
ALTER TABLE users
    ADD COLUMN last_login_country    TEXT CHECK (last_login_country IS NULL OR length(last_login_country) <= 64),
